...
- `src/hcrd/shape_inflection_confidence.py`: multiscale chord contrasts,
  simultaneous Gaussian bands, and confidence-set inversion.
...
- `tests/test_*scale_confidence.py` and
  ...
  coverage checks.
- `theory/hct/`: theorem statements, derivations, assumptions, and the proof
  DAG for the heteroskedastic extension.
...
bound. The current package does not claim arbitrary heteroskedastic,
non-Gaussian, or dependent-noise validity.
