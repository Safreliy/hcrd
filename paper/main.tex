...
\title{Hierarchical Convexity-Run Decomposition for Interpretable
LC--MS Peak-Quality Curation:\
...
nonlinear geometric transform, and evaluate it for expert curation of
pre-bounded LC--MS extracted-ion chromatograms.  Each level partitions the
current graph into runs of one discrete-curvature sign, replaces each run by
its endpoint chord, and recurses only on retained knots; signed deviations form
localized lobes.  We give complete pseudocode and prove exact reconstruction,
nested termination, linear knot-only work, affine equivariance, variation
control, and local and stochastic hierarchy-agreement guarantees.  The hard
knot map is globally discontinuous, motivating stable proximal-curvature and
signed-curvature-persistence companions.  Supporting Gaussian inference for
fixed or independent-guide templates provides finite-sample scan bounds and a
rank-aware projected-$\chi$ envelope for continuous families.  In frozen
no-refit transfers between two fully labelled studies, HCRD-8 plus an
independent reimplementation of a published two-component quality score
improved average precision by 0.1181 and 0.1005 (Holm-adjusted $p=0.00040$ in
both).  HCRD-8 had positive point differences over HCRD-1 in both directions,
with multiplicity-adjusted support in one.  In a conditionally labelled study,
it placed 7 of 17 residual bad features in the first 5\% of a review queue,
versus none for the reimplemented score.  Under a larger independent
TARDIS/FAME laboratory shift, the HCRD-8 gain was not confirmed and a compact
conventional bank was point-best.  An equal-dimensional Gaussian-derivative
control tied HCRD in one transfer, whereas HCRD was superior in the reverse.
These results identify a specific domain
for a simple, interpretable chord-lobe representation without claiming
universal signal-processing superiority.
\end{abstract}
...
\begin{enumerate}
  ...
  \item HCRD-specific local, deterministic-noise, and stochastic
        hierarchy-agreement guarantees, together with a sharp global
        discontinuity boundary and stable proximal/persistence companions;
