...
Detection Theory and Cross-Study Validation for Interpretable Chord Lobes}
...
nonlinear geometric transform for sampled signals.  Each level partitions the
current graph into maximal runs of one discrete-curvature sign, replaces every
run by its endpoint chord, and recurses only on retained knots; signed
deviations form localized lobes.  We prove exact reconstruction, nested finite
termination, linear total work, affine equivariance, variation control, and
local decision-cell stability.  Because the hard knot map is globally
discontinuous, we distinguish it from stable proximal-curvature and
signed-curvature-persistence companions.  For fixed or independent-guide lobe
dictionaries, Gaussian scan theory gives finite-sample detection and
localisation bounds with matching $\sqrt{\log M}$ lower rates.  A projected
$\chi$ norm provides exact rank-aware calibration for a continuous
location--scale--asymmetry family; independent replicates and buffered Gaussian
$m$-dependent splits provide valid adaptive tests with nested-tree family-wise
error control.  We evaluate the predicted application class: expert curation of pre-bounded
LC--MS extracted-ion chromatograms.  In frozen no-refit transfers between two
fully labelled studies, HCRD-8 plus the published quality score improved
average precision by 0.1181 and 0.1005 (Holm-adjusted $p=0.00040$ in both) and
exceeded HCRD-1.  A third, conditionally labelled study showed strong
residual-error reranking.  Under a larger independent TARDIS/FAME laboratory
shift, HCRD-8 improved the source score by 0.0725 only in point estimate, tied
HCRD-1, and did not beat a compact conventional feature bank. Results on
additional benchmarks further delimit the application domain. Together, these results
identify a specific application domain for a simple, interpretable chord-lobe
representation; broader signal-processing tasks did not show a consistent
...
\begin{enumerate}
  \item a precise operator separating the historical implementation from a
        centred inflection-localisation rule;
  \item finite-hierarchy, sparse linear-work, geometric, and stability results;
  \item a minimal discontinuity counterexample and explicit applicability
        boundaries;
  \item an impossibility theorem for continuous generic agreement and a
        globally nonexpansive proximal companion with irregular-grid
        certificates;
  \item a globally stable signed-curvature persistence metric that replaces
        unstable hard coordinates by same-sign lobe lifetimes;
  \item common-scale affine-null and multilevel hierarchy-agreement
        certificates plus an unconditional stochastic-margin bridge, a
        rate-optimal finite lobe-dictionary scan, exact ambient-rank calibration
        of a continuous triangular family, and exact replicate/buffered-split
        tests with nested-tree family-wise control;
  \item dimensionless lobe fullness and quadratic concentration descriptors,
        with area/energy treated as hierarchy summaries rather than as the
        decomposition itself;
  \item bidirectional cross-study transfer on expert-labelled LC--MS
        mass-feature quality, conditional residual-error reranking on Pttime,
        an independent double holdout, ablations comparing one level, the full
        hierarchy, scalar geometry, and area summaries, and evaluations in
        adjacent and cross-domain tasks.
\end{enumerate}
...
\end{equation}
Starting at the left endpoint, the knot walk passes insignificant curvatures
and active curvatures of one sign.  A sampled zero between opposite signs, or
otherwise the first opposite active curvature, closes the run.  The process is
restarted at that knot.  Let $K(y)$ denote the resulting knot set and let $Ty$
be linear interpolation through $(x_k,y_k)$ for $k\in K(y)$.
...
critical standardised lobe norm of order $\sqrt{\log M}$; the scan is
rate-optimal, with an explicit constant gap.
\end{theorem}
\begin{proof}
...
\begin{theorem}[Rank-aware exact continuum calibration]
\label{thm:rank-scan}
...
\end{equation}
A closed-form, dependency-free replacement for the exact quantile is
\begin{equation}
 \bar t_{q,\alpha}=
...
noise, the squared projected norm is $\chi_q^2$.  The true-template coordinate
is $N(\mu,1)$, giving the exact-normal power condition.  On the projected-norm
...
No entropy or discretisation is needed for
...
arbitrary dependence.  Assign $a_{\rm root}=\alpha$ and local levels satisfying
\begin{equation}
...
\section{Exact recovery task class}
\begin{corollary}[Chord-lobe recovery]
Suppose the first HCRD knot set is $K=\{k_r\}$, a baseline $b$ is affine on
each interval $[x_{k_r},x_{k_{r+1}}]$, and $q_k=0$ for every $k\in K$.  Then
$T(b+q)=b$ and $(I-T)(b+q)=q$.
\end{corollary}
\begin{proof}
At every selected knot, $b+q=b$.  Their chord interpolants therefore coincide.
\end{proof}
...
Wilson interval $[0.000276,0.000666]$.  Its exact-normal sufficient norm
5.1739 for $\beta=0.2$ gave empirical power 0.8773 in 10,000 draws.  For the
complete continuum, $q=127$: the exact chi-squared threshold 12.4218 was
exceeded in 2537/50,000 direct projected-norm draws, giving 0.05074
($[0.04883,0.05270]$), while the closed-form Laurent--Massart threshold
13.1150 was exceeded with probability 0.00486.  The resulting exact-normal
...
reported as ties, not losses.  On the exploratory variable-amplitude,
...
$1.07\times10^{-14}$.  EMD and VMD settings follow their standard algorithms
\citep{huang1998emd,dragomiretskiy2014vmd}; oracle tuning deliberately favours
the smoothing comparators.
...
post-hoc model search.  Our qscore was independently recomputed from the raw
mzML global feature boxes; on 1495 published Falkor features its Pearson
...
resamples both sample and candidate-ion groups; E2 intervals are paired feature
bootstraps.  F and M abbreviate Falkor and MESOSCOPE.}
...
placed in the top 19 (5\%) review positions, versus none under qscore.  HCRD-8
...
source-model-selected features.  AP-bad is primary; queue yield is the number
of 17 bad features in the top 19 review positions.}
\label{tab:lcms-e5}
\begin{tabular}{lrrr}
\toprule
Representation & AP-bad & ROC AUC-bad & Top-5\% yield \\
\midrule
qscore & 0.0391 & 0.3403 & 0/17 \\
DOMAIN+Q & 0.3873 & 0.8115 & 6/17 \\
HCRD-1+Q & 0.2845 & 0.7792 & 5/17 \\
HCRD-8+Q & \textbf{0.5085} & \textbf{0.8646} & \textbf{7/17} \\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{figures/exact_class_benchmark.pdf}
\caption{Baseline error on the theorem-matched noiseless class.  Centred HCRD
recovers the baseline to machine precision; thresholded HCRD and an
endpoint-only RDP solution tie it numerically and are therefore not labelled as
strictly inferior.}
\label{fig:benchmark}
\end{figure}
...
average precision over the published qscore by 0.1181 and 0.1005, with
Holm-adjusted $p=0.00040$ in both directions, and exceeded the one-level
variant.  E5 further supports residual-error reranking when only conditional
...
stochastic margins certify local hard-HCRD decisions.  The rank-aware scan,
...
radius; estimated versions, infinite-memory noise, and general polygonal
templates remain open.  Empirically, representation selection must be frozen
...
The accompanying repository contains the implementation, tests, experiment
protocols, compact outputs, source and feature hashes, and environment
instructions. Large third-party-derived feature caches are rebuilt locally and
are not redistributed. Code is licensed under MIT; the manuscript,
documentation, original figures, protocols, and original results are licensed
under CC BY 4.0.
