...
$1-\delta$.  Phase experiments comprising 206,000 signals followed the
predicted transition and recovered every boundary in all 80,000 draws from the
strict certified regions.  Structural results establish finite termination, affine
equivariance, variation control, global discontinuity of the hard map, and
stable companions.  In frozen no-refit transfers between two fully labelled
LC--MS studies, HCRD-8 plus an independently reimplemented quality score
improved average precision by 0.1181 and 0.1005.  An equal-dimensional
Gaussian-derivative control tied HCRD in one direction, whereas HCRD was
superior in the reverse; a larger TARDIS/FAME shift did not confirm
superiority.  HCRD therefore provides a simple morphological inductive bias
with a quantitative recovery regime and explicit application boundaries.
\end{abstract}
...
The approximate-join departure experiment varied both
...
An initial double-group holdout on the expert-labelled EIC corpus of
\citet{muller2020eic} gave the largest AP to raw plus HCRD-8 (0.4092 versus
...
The prospective E2 bootstrap is conditional on the fitted source-study model.
We therefore added a fixed secondary sensitivity that resamples source and
target retention-time blocks, refits the source learner in every replicate,
and varies block width from 30 to 120 seconds.  A separate ten-fold
...
Across the exact- and approximate-join phase experiments, all 80,000 draws in
the strict certified regions recovered every boundary.  Outside the sufficient
...
Source-refit RT-block means were also positive for all tested block widths,
although their percentile intervals included zero; hence the prospective E2
claim remains the conditional no-refit transfer rather than an unconditional
