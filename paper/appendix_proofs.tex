...
\begin{proof}[Detailed completion of Theorem~\ref{M-thm:finite} and Corollary~\ref{M-cor:sparse}]
...
Theorem~\ref{M-thm:local-stability}.
If
...
\begin{proof}[Detailed completion of Theorem~\ref{M-thm:local-stability}]
...
\begin{lemma}[Essential class in the signed persistence distance]
\label{lem:essential}
...
\begin{proof}[Detailed completion of Theorem~\ref{M-thm:persistence}]
...
$d_{\rm SC}$.  Taking the maximum over signs proves the displayed distance
bound.  Finally,
a finite bar $(b,d)$ has diagonal cost $(b-d)/2$; if its lifetime is strictly
greater than twice that bound, a bottleneck matching attaining the bound
cannot send it to the diagonal.
\end{proof}

\begin{proof}[Domain detail for Theorem~\ref{M-thm:prox}]
