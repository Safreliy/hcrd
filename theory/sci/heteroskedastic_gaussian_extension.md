...
The proof uses two standard Gaussian facts. First, Anderson's inequality says
that shifting a centred Gaussian vector cannot increase its probability of
falling in a convex set that is symmetric about zero. This statement also
allows a singular covariance matrix, which matters because `R` is a
projection. Applied to a Euclidean ball, it gives, for every \(q\geq0\),
...
Second, diagonalize the positive semidefinite matrix `A`. Then
`||Z||_2^2` has the same distribution as
`sum_j lambda_j G_j^2`, where the `G_j` are independent standard normal
variables. The weighted chi-square lower-tail inequality gives, with
...
If `T=0`, the trace bound below implies that every variance is zero and the
claim is immediate. Assume from now on that `T>0`. Every diagonal element of a
block residual projector is
\(1-|B|^{-1}\geq1/2\), so
...
The two external inequalities used here are Anderson's lemma for centred
Gaussian measures and the Laurent--Massart lower-tail bound for weighted
chi-square variables. A submission version should cite their exact theorem
statements rather than refer only to their names.
