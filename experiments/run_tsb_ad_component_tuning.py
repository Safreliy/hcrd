"""Tune A3 learned anomaly scores over signed HCRD components."""
...
from hcrd import hcrd_component_iforest_scores, vus_pr_roc  # noqa: E402
...
def _process_file(task: tuple[str, str]) -> dict[str, object]:
    ...
    train_size = int(filename.removesuffix(".csv").split("_")[-3])
    window = int(find_length_rank(signal.reshape(-1, 1), rank=1))
    started = time.perf_counter()
    candidates = hcrd_component_iforest_scores(
        signal, train_size=train_size, max_levels=8
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=ROOT / "results" / "tsb_ad_a3"
    )
...
def main() -> None:
    ...
    a2_freeze = ROOT / "results" / "tsb_ad_a2" / "frozen_configuration.json"
    if not a1_freeze.exists() or not a2_freeze.exists():
        raise RuntimeError("A1 and A2 must be frozen before A3 tuning")
    ...
    if args.limit is None:
        ...
        implementation_files = [
            ROOT / "src" / "hcrd" / "components.py",
            ROOT / "src" / "hcrd" / "learned_anomaly.py",
            ROOT / "src" / "hcrd" / "anomaly.py",
            ROOT / "src" / "hcrd" / "tsad_metrics.py",
            Path(__file__).resolve(),
            ROOT / "docs" / "tsb_ad_a3_protocol.md",
        ]
        frozen = {
            "status": "frozen_before_A1_A2_A3_evaluation_execution",
            ...
            "a2_frozen_configuration_sha256": _sha256(a2_freeze),
