"""Frozen fresh-seed E2 comparison against oracle slow-tail CEEMDAN."""
...
OUTPUT = PROJECT / "results" / "ceemdan_confirmation_e2"
sys.path.insert(0, str(PROJECT / "src"))

from hcrd.core import decompose  # noqa: E402
from hcrd.external_baselines import ceemdan_slow_tail_path  # noqa: E402
...
def _run_trial(task: tuple[int, float, int]) -> list[dict[str, object]]:
    ...
    candidates, reconstruction_error = ceemdan_slow_tail_path(
        signal.observed,
        signal.x,
        trials=CEEMDAN_TRIALS,
        noise_seed=100_000_000 + signal_seed,
        maximum_tail_components=4,
    )
    ceemdan_seconds = time.perf_counter() - started
    tail_count, oracle = _oracle(candidates, signal.baseline)
    estimates = (
        ("hcrd_centered", "none", centred),
        ("hcrd_thresholded", "z=3.5", thresholded),
        ("ceemdan_final", "tail=1", candidates[0]),
        ("ceemdan_oracle_tail", f"tail={tail_count}", oracle),
    )
    return [
        {
            "noise_sigma": noise_sigma,
            ...
            "ceemdan_noise_seed": 100_000_000 + signal_seed,
            "method": method,
            "parameter": parameter,
            "baseline_mse": mse(estimate, signal.baseline),
            "ceemdan_reconstruction_error": reconstruction_error,
            "ceemdan_seconds": ceemdan_seconds,
        }
...
def main() -> None:
    protocol = PROJECT / "docs" / "ceemdan_confirmation_protocol.md"
    if not protocol.exists():
        raise FileNotFoundError("frozen E2 protocol is missing")
    tasks = [
    ...
    rows.sort(key=lambda row: (float(row["noise_sigma"]), int(row["trial"]), str(row["method"])))
    ...
    for comparison_index, noise_sigma in enumerate(NOISE_LEVELS):
        ...
        competitor = np.asarray(
            [
                ...
                for row in rows
                if float(row["noise_sigma"]) == noise_sigma
                and row["method"] == "ceemdan_oracle_tail"
            ]
        )
        ...
        comparisons.append(
            {
                ...
                "competitor": "ceemdan_oracle_tail",
    ...
    metadata = {
        ...
        "emd_signal": importlib.metadata.version("EMD-signal"),
        "maximum_reconstruction_error": max(
            float(row["ceemdan_reconstruction_error"]) for row in rows
        ),
    }
